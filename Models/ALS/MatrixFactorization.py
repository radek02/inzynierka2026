...
ratings_coo = coo_matrix(
    (df["rating"].astype(float), (df["user"], df["book"])),
...
user_id_map = df[["user", "user_id"]].drop_duplicates().sort_values("user") # index_csv -> index_als user = index_als | user_id = index_csv
